...
class DatecsGenerate {
  ...
  image(String base64, double size) {
    args.add("img%2021" + size.toString() + "%2021" + base64);
  }

  row(List<DatecsColumn> cols) {
    final isSumValid =
        cols.fold(0, (int sum, col) => sum + col.width) == getDivider();
    if (!isSumValid) {
    ...
    for (int i = 0; i < cols.length; ++i) {
      ...
      if (cols[i].text.length > max_char_col) {
        ...
        if (cols[i].styles.align == DatecsAlign.left ||
            cols[i].styles.align == DatecsAlign.center) {
